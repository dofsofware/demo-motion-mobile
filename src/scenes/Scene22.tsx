...
export const Scene22 = ({ inFrame, outFrame, crossfadeFrames }: P) => {
  ...
  // ---------- Profit section fade-in (fadeSlideUp) ----------
  ...
  const sectionTranslateY = interpolate(sectionProgress, [0, 1], [60, 0]);
  ...
  // Use easeOutQuad (1 - (1-t)^2)
  const easeOut = (t: number) => 1 - Math.pow(1 - t, 2);
  const easedProgress = easeOut(metricsProgress);

  // Target values
  const revenueTarget = 45_800_000; // 45.8M FCFA
  const costsTarget = 18_700_000;   // 18.7M FCFA
  ...
  // Bar widths (as percentages of max)
  ...
  // Trend arrow bounce (bounceRight) and green pulse continuous
  const bounceX = Math.sin(time * Math.PI * 2) * 5; // 1s cycle, but original bounceRight is 1s infinite with max 10px
  // Actually bounceRight: translateX 0 -> 10px -> 0 over 1s, we'll simulate with sine.
  const bounceXValue = Math.sin(time * 2 * Math.PI) * 5; // 1s cycle, amplitude 5
  // Pulse green on trend icon: scale 1 to 1.1, opacity 0.8 to 1, 1.4s cycle
          ...
          >
            ...
            >
              <div style={{ display: "flex", alignItems: "baseline", gap: 20, flexWrap: "wrap" }}>
                <div style={{ fontSize: 36, fontWeight: 600, color: "#0F2B6D" }}>Rentabilité projetée</div>
                <div
                  style={{
                    fontSize: 56,
                    fontWeight: 800,
                ...
                <div style={{ fontSize: 28, color: "#2C3E66", marginTop: 8 }}>
