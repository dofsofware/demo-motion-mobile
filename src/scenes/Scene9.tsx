...
export const Scene9 = ({ inFrame, outFrame, crossfadeFrames }: P) => {
  ...
  // ---------- Rocket icon entrance (scalePopSpin) ----------
  const rocketEntrance = interpolate(frame, [0, 0.9 * fps], [0, 1], {
    ...clamp,
    easing: Easing.bezier(0.2, 0.9, 0.4, 1.1),
  });
  const rocketOpacity = interpolate(rocketEntrance, [0, 0.5, 1], [0, 1, 1], clamp);
  const rocketScale = interpolate(rocketEntrance, [0, 0.5, 1], [0.2, 1.08, 1], clamp);
  const rocketRotate = interpolate(rocketEntrance, [0, 0.5, 1], [-120, 3, 0], clamp);
  const rocketBlur = interpolate(rocketEntrance, [0, 0.5, 1], [10, 0, 0], clamp);

  // Rocket continuous float (3s cycle)
  const rocketFloatY = Math.sin((time / 3) * Math.PI * 2) * 4;
  const rocketFloatRotate = Math.sin((time / 3) * Math.PI * 2) * 1;
  ...
  // Word "transforme" pulse (2.2s cycle)
      ...
      >
        ...
        <div
          style={{
            ...
            transform: `translateY(${rocketFloatY}px) rotate(${rocketFloatRotate}deg) scale(${rocketScale}) rotate(${rocketRotate}deg)`,
